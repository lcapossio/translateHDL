...
module counter_trace_tb;
    ...
    reg         en;
    wire [7:0]  count;
    // 4096 bits = 512 chars. A `reg [1023:0]` holds only 128, and
    // $value$plusargs silently FRONT-TRUNCATES a longer path, so a deep
    // checkout fails with "VCD Error: Unable to open ..." - which reads
    // like a waveform divergence but is purely a path-length limit.
    reg [4095:0] wave_path;
    integer     i;
