...
def compare(manifest_path: str) -> LayerResult:
    ...
    res = LayerResult("L4 synth", N_A)

    spec = man.get("synth")
    if not spec or not spec.get("enabled", True):
        # N_A, not PASS: nothing was compared (see formal_equiv.prove).
        res.detail = "no synth section - layer not configured (no evidence either way)"
        return res
    if not have("yosys"):
