...
def prove(manifest_path: str) -> LayerResult:
    ...
    res = LayerResult("L2 formal", N_A)

    spec = man.get("formal")
    if not spec or not spec.get("enabled", True):
        # N_A, NOT pass. This layer IS the proof; returning PASS for a proof
        # that was switched off would report "equivalent" for a design nothing
        # ever checked. N_A carries no evidence, so the orchestrator cannot
        # fold it into a passing verdict. (SKIP means "a tool was missing";
        # N_A means "you configured this layer not to run" - both honest,
        # neither a pass.)
        res.detail = "no formal section - layer not configured (no evidence either way)"
        return res
