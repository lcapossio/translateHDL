...
def timescale_to_ps(text: str) -> float:
    match = re.search(r"\$timescale\s+(\d+)\s*([fpnum]?s)\s+\$end", text, re.DOTALL)
    if not match:
...
def compare(manifest_path: str) -> LayerResult:
    ...
    res = LayerResult("L3b waveform", N_A)

    spec = (man.get("simulation") or {}).get("waveform")
    if not spec:
        # N_A, not PASS: "nothing to compare" is not "the waveforms agree".
        res.detail = "no simulation.waveform section - layer not configured"
        return res
    ...
    try:
        ...
        # ToolMissing from either side propagates untouched: cli_main / the
        # orchestrator turn it into SKIP, and the outer `finally` still cleans up.
