...
def compare(manifest_path: str) -> LayerResult:
    ...
    res = LayerResult("L3a trace", N_A)

    spec = (man.get("simulation") or {}).get("trace")
    if not spec:
        # N_A, not PASS: "nothing to compare" is not "the traces agree".
        res.detail = "no simulation.trace section - layer not configured"
        return res
    ...
    try:
        ...
        # ToolMissing propagates untouched here - cli_main / the orchestrator
        # turn it into SKIP.
        if tmp is not None:
            shutil.rmtree(tmp, ignore_errors=True)
