...
def parse_verilog(text: str, top: str) -> tuple[dict[str, Port], dict[str, str]]:
    ...
    m = re.search(rf"\bmodule\s+{re.escape(top)}\b(.*?)\bendmodule\b", text, re.DOTALL)
    if not m:
        raise ValueError(f"verilog module '{top}' not found")
    body = m.group(1)

    params: dict[str, str] = {}
    pm = re.search(r"#\s*\((.*?)\)\s*\(", body, re.DOTALL)
    if pm:
...
def parse_vhdl(text: str, top: str) -> tuple[dict[str, Port], dict[str, str]]:
    text = _strip_comments(text)
    m = re.search(rf"\bentity\s+{re.escape(top)}\s+is\b(.*?)\bend\b", text, re.DOTALL | re.IGNORECASE)
    if not m:
        raise ValueError(f"vhdl entity '{top}' not found")
    body = m.group(1)

    params: dict[str, str] = {}
    gm = re.search(r"\bgeneric\s*\((.*?)\)\s*;", body, re.DOTALL | re.IGNORECASE)
    if gm:
    ...
    ports: dict[str, Port] = {}
    pm = re.search(r"\bport\s*\((.*)\)\s*;", body, re.DOTALL | re.IGNORECASE)
    if pm:
        depth_text = pm.group(1)
        for decl in re.split(r";", depth_text):
            dmatch = re.match(r"\s*([\w\s,]+?)\s*:\s*(in|out|inout)\b\s*(.*)", decl, re.DOTALL | re.IGNORECASE)
            if not dmatch:
                continue
            names, direction, typ = dmatch.group(1), dmatch.group(2).lower(), dmatch.group(3).strip()
            width = None
            wm = re.search(r"\(\s*(\d+)\s+downto\s+(\d+)\s*\)", typ, re.IGNORECASE)
            if wm:
                width = abs(int(wm.group(1)) - int(wm.group(2))) + 1
            elif re.search(r"\bstd_u?logic\b", typ, re.IGNORECASE) and "(" not in typ:
                width = 1
            for name in (n.strip() for n in names.split(",")):
                if name:
